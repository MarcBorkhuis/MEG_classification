...
import numpy as np
import torch
from torch.utils.data import DataLoader, TensorDataset, Subset # Subset is used by parent's split_data
...
class CustomMEGDataset(ContinuousDataset):
    ...
    def __init__(self, data_root, scenario, mode, task_to_label_map,
        ...
        sfreq : float, optional
            Sampling frequency of the MEG data in Hz. Default is 2034.
        ...
        self.string_target_labels = sorted(task_to_label_map.keys(), key=lambda k: task_to_label_map[k])
        ...
        self.groups = torch.empty(0, dtype=torch.long) # Integer subject IDs for splitting
    ...
    def _get_data_paths(self):
        ...
        if self.scenario == "intra":
            if self.mode == "train":
                ...
                raise ValueError(f"Invalid mode '{self.mode}' for 'intra' scenario. Expected 'train' or 'test'.")
            ...
            if self.mode == "train":
                ...
                # Critical configuration error
                raise ValueError(f"Invalid mode '{self.mode}' for 'cross' scenario. Expected 'train', 'test1', 'test2', or 'test3'.")
        else:
            # Critical configuration error
            raise ValueError(f"Invalid scenario: {self.scenario}. Expected 'intra' or 'cross'.")
        
        # Validate that determined paths actually exist
        valid_paths = []
        for p in paths:
            if not os.path.isdir(p):
                LOG.warning(f"Data directory not found: {p}. This path will be skipped.")
            else:
    ...
    def _split_window(self, sub_data_np):
        ...
        if sub_data_np.ndim == 1: 
             LOG.warning(f"Input to _split_window is 1D (shape: {sub_data_np.shape}). Assuming single channel data.")
             sub_data_np = sub_data_np.reshape(1, -1)
        elif sub_data_np.ndim > 2: 
             LOG.warning(f"Input to _split_window has >2 dimensions (shape: {sub_data_np.shape}). Expected (channels, timepoints). Taking first 2D slice.")
             sub_data_np = sub_data_np[0] # Assuming the first slice is the relevant (channels, timepoints) data
        
        if sub_data_np.ndim != 2:
            LOG.error(f"Input data for windowing must be 2D (channels, timepoints), but got shape {sub_data_np.shape}. Cannot proceed.")
            return torch.empty(0) # Return empty tensor if shape is unrecoverable
        ...
        step_samples = int(window_samples * (1 - self.overlap)) # Step size based on overlap
        ...
        for i in range(offset_samples, total_timepoints - window_samples + 1, step_samples):
            trial = sub_data_np[:, i : i + window_samples] # Extract a window: (channels, window_samples)
            
            # Skip trial if it contains NaN values
            if np.isnan(trial).any():
                LOG.debug("NaNs found in a trial segment; skipping this trial.") # Debug level, can be noisy
                continue
            ...
            scaled_trial = np.array([self.scaler(channel_data) for channel_data in trial]) # Shape: (channels, window_samples)
        ...
        return torch.from_numpy(np.array(windows)).float() # Shape: (num_windows, 1, channels, window_samples)
    ...
    def _get_h5_dataset_name(self, filepath):
        ...
        if len(parts) > 1: # Ensure there's at least "name_chunk" structure
            ...
            return parts[0] # Return the part before any potential extension/chunk
    ...
    def _extract_info_from_filename(self, filename):
        ...
        if subject_id_str.isdigit():
            ...
            if not task_type_str: # Should not happen if len(parts) >= 3
                LOG.error(f"Extracted empty task type from filename: {filename}")
                return None, None
            ...
            for i in range(len(parts) - 2, -1, -1):
                if parts[i].isdigit() and len(parts[i]) >= 5: # Heuristic: subject ID is numeric, length >=5
                    subject_id_candidate = parts[i]
                    task_type_parts_candidate = parts[:i] # Parts before subject ID form the task
                    break 
            ...
            LOG.error(f"Failed to robustly extract numeric subject ID from filename: {filename}. Parts: {parts}")
            return None, None
            ...
            with h5py.File(filepath, 'r') as f:
                ...
                if h5_dataset_name not in f:
                    ...
                    if len(keys_in_file) == 1:
                        ...
                        LOG.debug(f"Derived H5 dataset name not found. Using single available key '{h5_dataset_name}' for H5 file {filepath}")
                    else:
                        LOG.error(f"Dataset key '{h5_dataset_name}' not found in H5 file: {filepath}. Available keys: {keys_in_file}")
                        return None
    ...
    def _get_or_create_subject_int_id(self, subject_str_id):
        ...
        if subject_str_id not in self._subject_str_to_int_map:
            ...
            self._int_to_subject_str_map[new_id] = subject_str_id # Store reverse mapping
    ...
    def load_all_data(self):
        ...
        if not self.data_paths_to_load:
            LOG.warning(f"No valid data paths found for scenario='{self.scenario}', mode='{self.mode}'. Dataset will be empty.")
        
        for data_dir in self.data_paths_to_load:
            LOG.info(f"Processing data directory: {data_dir}") # Keep this for clarity on which dir is processed
            try:
                # List all .h5 files in the current directory
                filenames = [f for f in os.listdir(data_dir) if f.endswith(".h5")]
            except FileNotFoundError:
                LOG.error(f"Data directory '{data_dir}' not found during os.listdir. Skipping."); continue
            
            if not filenames:
                LOG.warning(f"No .h5 files found in directory: {data_dir}"); continue

            for filename in filenames:
                filepath = os.path.join(data_dir, filename)
                task_type_str, subject_id_str = self._extract_info_from_filename(filename)

                # Validate parsing results
                if not (task_type_str and subject_id_str):
                    LOG.warning(f"Could not parse info from filename '{filepath}'. Skipping."); continue
                ...
                if task_type_str not in self.task_to_label_map:
                    LOG.warning(f"Unrecognized task type '{task_type_str}' in filename '{filepath}'. Skipping."); continue
                
                target_label_int = self.task_to_label_map[task_type_str]
                subject_id_int = self._get_or_create_subject_int_id(subject_id_str)
                
                # Load raw data from H5 file
                continuous_data_np = self._load_h5_file_data(filepath)
                if continuous_data_np is None: continue # Error already logged by _load_h5_file_data
                ...
                if windowed_data_torch is None or len(windowed_data_torch) == 0:
                    LOG.warning(f"No valid windows generated from file '{filepath}'. Skipping."); continue
                ...
                all_targets_list.extend([target_label_int] * num_windows_from_file)
                all_groups_list.extend([subject_id_int] * num_windows_from_file)
        ...
        if all_windows_list:
            ...
            LOG.warning(f"No data successfully loaded for scenario='{self.scenario}', mode='{self.mode}'. Dataset is empty.")
            # Initialize with correct (empty) shapes to prevent downstream errors
            num_window_samples = int(self.sfreq * self.window) if self.sfreq and self.window > 0 else 1
    ...
    def __getitem__(self, idx):
        ...
        if not isinstance(idx, int):
            if isinstance(idx, (list, torch.Tensor)) and len(idx) == 1:
                idx = idx[0].item() if isinstance(idx, torch.Tensor) else idx[0] # Convert to int
            else:
                # This case should ideally not be hit with standard DataLoader usage.
                raise TypeError(f"Index must be int or a single-element list/tensor, not {type(idx)}")

        # Boundary check for the index
        if not (0 <= idx < len(self.data)):
            LOG.error(f"Index {idx} out of bounds for dataset of length {len(self.data)}")
